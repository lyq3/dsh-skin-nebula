...
深海军蓝半透明表面 + 霓虹青/电紫点缀，配一张 AI 生成的高清星云背景（暗色主题）与极光薄雾背景（亮色主题）。表面层做了半透明处理，星云从内容后面透出来；弹层与菜单保持近不透明以保证可读性。
...
- `assets/` — codex（gpt-image-2）生成的 1536×1024 高清背景：`nebula-dark.png` / `nebula-light.png`。
